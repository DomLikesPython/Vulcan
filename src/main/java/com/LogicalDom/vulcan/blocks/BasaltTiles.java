...
public class BasaltTiles extends Block {
    public BasaltTiles() {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if(!worldIn.isRemote()) {
            ...
            if(stack.getItem().equals(Registries.FORGE_HAMMER.get())) {
                player.getEntityWorld().setBlockState(pos, Registries.BASALT_BRICKS.get().getDefaultState());
                worldIn.playSound(player, pos.getX(), pos.getY(),
                        pos.getZ(), SoundEvents.BLOCK_ANVIL_HIT, SoundCategory.BLOCKS, 20f, 1f);
